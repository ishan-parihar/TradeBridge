...
class TradingLimits:
    ...
    """Prevents death-by-a-thousand-cuts pattern seen in audit."""

    min_rest_between_trades_sec: int = 300
    """5-minute minimum between trades — prevents impulsive re-entry."""

    # Loss limits
    max_loss_per_day_pct: float = 2.0
    """Max daily loss as % of equity. $4.04 on $202 account."""

    max_loss_per_trade_pct: float = 2.0
    """Max risk per single trade as % of equity."""

    # Consecutive loss handling
    cooldown_after_consecutive_losses: int = 2
    """After N consecutive losses, trigger mandatory cool-off."""

    cooldown_duration_after_losses_sec: int = 1800
    """30-minute mandatory cool-off after consecutive losses."""

    # Confluence requirements
    require_indicator_confluence: bool = True
    """Require 2+ indicators agreeing before entry."""

    min_confluence_count: int = 2
    """Minimum number of indicators that must agree."""
    ...
    # Breakeven rules
    min_profit_before_breakeven_atr: float = 1.0
    """Don't move SL to BE until price moves 1x ATR in favor.
    Premature BE caused 2 whipsaw losses in audit session."""
...
class TradingPolicy:
    ...
    def validate_submit_order(
        ...
        if self._consecutive_losses >= self.limits.cooldown_after_consecutive_losses:
    ...
    def validate_breakeven_move(
        ...
        Audit finding: premature breakeven stops caused 2 whipsaw losses.
        Rule: don't move to BE until price moves 1x ATR in favor.
        """
        if atr_points <= 0:
            return PolicyDecision(
                allowed=False,
