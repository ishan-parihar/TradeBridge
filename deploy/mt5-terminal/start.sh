...
# ── Step 5: Install MT5 (first run only) ─────────────────────────────────────
if [ -f "$MT5_EXE" ]; then
    echo "[5] MT5 already installed at $MT5_EXE"
else
    echo "[5] Downloading MT5 installer..."
    mkdir -p "$MT5_DIR"
    curl -sL "$MT5SETUP_URL" -o /tmp/mt5setup.exe
    echo "[5] Installing MT5 silently (this may take 2-5 minutes)..."
    wine /tmp/mt5setup.exe /auto 2>/dev/null || true
    rm -f /tmp/mt5setup.exe
    sleep 3
    if [ -f "$MT5_EXE" ]; then
        echo "[5] MT5 installed successfully"
    else
        echo "[5] WARNING: MT5 may not have installed. Checking alternatives..."
        find "$WINEPREFIX/drive_c" -name "terminal64.exe" 2>/dev/null | head -3
    fi
fi
